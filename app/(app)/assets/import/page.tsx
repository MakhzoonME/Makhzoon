      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-4">
        ...
        {!fileName ? (
          ...
          <div className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2 border border-gray-200">
            ...
            <button onClick={reset} className="text-gray-400 hover:text-gray-600 flex-shrink-0 ml-2">
              <X className="h-4 w-4" />
            </button>
          </div>
        )}
              ...
              <tbody>
                {preview.map((r, i) => (
                  <tr key={i} className="border-b border-gray-100 last:border-0">
                    {headers.map((h) => (
                      <td key={h} className="px-3 py-2 text-gray-700 whitespace-nowrap max-w-[200px] truncate">{r[h] || '—'}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
      ...
      {result && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <div className="px-5 py-3 border-t border-gray-200 flex justify-end gap-2">
            ...
            <Button size="sm" onClick={() => router.push('/assets')}>View assets</Button>
          </div>
        </div>
      )}
    </div>
