...
function CheckSVG() {
  return (
    <svg width="15" height="15" viewBox="0 0 15 15" fill="none" aria-hidden>
      ...
      <path d="M4.5 7.5l2 2 4-4" stroke="var(--primary-600)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
export default function ProductPage() {
  return (
    <>
      {/* Hero */}
      <section style={{ padding: '88px 32px 48px', textAlign: 'center', background: 'linear-gradient(180deg, #fff 0%, var(--surface-page) 100%)' }}>
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', position: 'relative' }}>
          ...
          <h1 style={{ fontSize: 'clamp(40px,5.5vw,60px)', fontWeight: 700, letterSpacing: '-0.025em', lineHeight: 1.06, margin: '0 0 22px', color: 'var(--gray-900)' }}>
            Built for the entire<br />asset lifecycle.
          </h1>
          <p style={{ fontSize: 19, color: 'var(--gray-600)', lineHeight: 1.55, margin: 0, maxWidth: 600, marginLeft: 'auto', marginRight: 'auto' }}>
            Acquire, assign, maintain, audit, retire. Five stages, one platform, zero handoffs to spreadsheets.
          </p>
        </div>
      </section>

      {/* Pillars */}
      <section style={{ background: '#fff', padding: '32px 32px 96px', position: 'relative' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 96 }}>
          {PILLARS.map(({ kicker, title, body, bullets, Preview, flip }) => (
            <div
              key={kicker}
              style={{
                display: 'grid',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {bullets.map(b => (
                    <div key={b} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 14, color: 'var(--gray-700)' }}>
                      <span style={{ flexShrink: 0 }}><CheckSVG /></span>{b}
                    </div>
                  ))}
                </div>
              </div>
