  ...
  return (
    <div>
      <PageHeader
        ...
        breadcrumb={[{ label: t('nav.assets'), href: `/${locale}/${orgSlug}/${space}/usool` }, { label: t('audits.breadcrumb'), href: `/${locale}/${orgSlug}/${space}/usool/audits` }]}
        actions={isAdmin ? (
          <Button size="sm" onClick={() => router.push(`/${locale}/${orgSlug}/${space}/usool/audits/new`)}>
      ...
      {audits.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-gray-400">
          ...
          <p className="mt-3 text-sm text-center">{t('audits.noAudits')}</p>
        </div>
      ) : (
          ...
          {audits.map((audit: InventoryAudit) => {
            ...
            return (
              <div
                ...
                onClick={() => router.push(`/${locale}/${orgSlug}/${space}/usool/audits/${audit.id}`)}
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 text-xs text-gray-500">
                      ...
                      {audit.completedAt && <span>· {t('audits.completedAt').replace('{date}', formatDate(audit.completedAt))}</span>}
                      <span>· {t('audits.itemCount').replace('{count}', String(audit.totalAssets))}</span>
                    </div>
                  </div>
