...
export default function WarrantiesPage() {
  ...
  const columns: ColumnDef<Warranty>[] = [
    { key: 'assetId', header: t('col.asset'), sortable: true, render: (w) => <button className="text-primary-600 hover:underline" onClick={() => router.push(`/${locale}/${orgSlug}/${space}/usool/${w.assetId}`)}>{w.assetName ?? w.assetId}</button> },
    ...
    { key: 'status', header: t('col.status'), sortable: true, render: (w) => <StatusBadge status={getWarrantyStatus(w.endDate)} /> },
    {
      ...
      render: (w) => (
        <Button size="sm" variant="ghost" onClick={() => router.push(`/${locale}/${orgSlug}/${space}/usool/${w.assetId}`)}>{t('warranties.viewAsset')}</Button>
      )
    },
  ];
