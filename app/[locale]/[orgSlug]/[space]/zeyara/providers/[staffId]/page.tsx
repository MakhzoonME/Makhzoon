...
// Provider detail — Zeyara's equivalent of Haraka's staff/[staffId] page.
// Not a thin shell over a shared body (the Haraka original hard-codes 'pos' /
// 'haraka' inline rather than reading useVertical()), so this is a small
// adapted copy scoped to the zeyara feature key / permission namespace and
// the /zeyara/... route root.
...
import { useStaff, useAppointments } from '@/hooks/haraka';
...
export default function ZeyaraProviderDetailPage() {
  ...
  const base = `/${params.locale}/${params.orgSlug}/${params.space}/zeyara`;
  ...
  if (!person) {
    return (
      <div className="p-6">
        ...
        <Button variant="outline" onClick={() => router.push(`${base}/providers`)}>
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumb={[
          ...
          { label: params.space },
          { label: t('nav.zeyara'), href: base },
          { label: t('staff.title'), href: `${base}/providers` },
          { label: person.name },
        ]}
        actions={
          <div className="flex items-center gap-2">
            {person.capabilities.includes('appointment_provider') && (
              <Button variant="outline" onClick={() => router.push(`${base}/providers/${person.id}/availability`)}>
                <CalendarClock className="h-4 w-4 me-2" strokeWidth={1.75} />
                {t('staff.availability')}
              </Button>
            )}
            <Button variant="ghost" onClick={() => router.push(`${base}/providers`)}>
              <ArrowLeft className="h-4 w-4 me-2" /> {t('common.back')}
            </Button>
          </div>
        }
