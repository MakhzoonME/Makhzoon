...
export default function AssetsPage() {
  ...
  const columns: ColumnDef<Asset>[] = [
    { key: 'name', header: t('col.name'), sortable: true, render: (a) => <button className="font-medium text-primary-600 hover:text-primary-700 hover:underline text-left" onClick={() => router.push(`/${locale}/${orgSlug}/assets/${a.id}`)}>{a.name}</button> },
      ...
      render: (a) => (
        <div className="flex items-center gap-1">
          {isAdmin ? (
            ...
            <Button size="sm" variant="ghost" onClick={(e) => { e.stopPropagation(); router.push(`/${locale}/${orgSlug}/assets/${a.id}`); }}>{t('assets.view')}</Button>
          )}
        </div>
      )
    },
    ...
    <div>
      ...
      <div className="bg-surface-card rounded-lg border border-border">
        <DataTable
          ...
          onRowClick={(a) => router.push(`/${locale}/${orgSlug}/assets/${a.id}`)}
          keyExtractor={(a) => a.id}
