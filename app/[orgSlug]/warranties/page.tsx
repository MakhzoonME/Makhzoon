...
export default function WarrantiesPage() {
    ...
    { key: 'status', header: 'Status', render: (w) => <StatusBadge status={getWarrantyStatus(w.endDate)} /> },
    {
      ...
      render: (w) => (
        <div className="flex gap-1">
          {isAdmin ? (
            <>
              <Button size="sm" variant="ghost" onClick={(e) => { e.stopPropagation(); router.push(`/${orgSlug}/warranties/${w.id}/edit`); }}><Edit className="h-3.5 w-3.5" /></Button>
  ...
  return (
    <div>
      <PageHeader
        title="Warranties"
        actions={isAdmin ? <Button size="sm" onClick={() => router.push(`/${orgSlug}/warranties/new`)}><Plus className="h-4 w-4 mr-1" />Add Warranty</Button> : undefined}
      />
      <FilterBar
