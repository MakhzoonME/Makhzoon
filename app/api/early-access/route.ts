  ...
  <body style="font-family:system-ui,sans-serif;background:#f9fafb;margin:0;padding:40px 0;">
    <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:12px;border:1px solid #e5e7eb;padding:40px;">
      ...
      <div style="background:#f3f4f6;border-radius:8px;padding:16px 20px;margin-bottom:24px;">
        ${firstName ? `<span style="font-size:13px;color:#6b7280;">Name</span><br/><span style="font-size:15px;font-weight:600;color:#111827;">${firstName}</span><br/><br/>` : ''}
...
const confirmHtml = (email: string, firstName?: string) => `
<!DOCTYPE html>
<html>
  <body style="font-family:system-ui,sans-serif;background:#f9fafb;margin:0;padding:40px 0;">
    <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:12px;border:1px solid #e5e7eb;padding:40px;">
      <div style="text-align:center;margin-bottom:32px;">
        ...
        <div style="font-size:22px;font-weight:700;color:#111827;margin-top:16px;letter-spacing:-0.02em;">You're on the list.</div>
        <div style="font-size:14px;color:#6b7280;margin-top:6px;">We'll be in touch soon.</div>
      </div>
      <p style="font-size:15px;color:#374151;line-height:1.6;margin:0 0 20px;">
        ${firstName ? `Hi ${firstName},<br/><br/>` : 'Hi there,<br/><br/>'}
      ...
      <p style="font-size:15px;color:#374151;line-height:1.6;margin:0 0 32px;">
        We're working hard to get the product ready and will reach out to <strong>${email}</strong> as soon as we're ready to onboard you.
      </p>
...
export async function POST(req: NextRequest) {
  ...
  await Promise.all([
    ...
    createEarlyAccessEntry(email, clientIp, firstName, lastName),
  ]);
