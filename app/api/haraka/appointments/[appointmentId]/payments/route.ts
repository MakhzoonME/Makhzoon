...
import { addAppointmentPaymentSchema } from '@/lib/modules/haraka/appointments/schemas'
...
export async function GET(
  ...
  try {
    ...
    console.error('[GET /api/haraka/appointments/[appointmentId]/payments]', err)
...
export async function POST(
  ...
  try {
    ...
    const parsed = addAppointmentPaymentSchema.safeParse(body)
    if (!parsed.success) {
      return NextResponse.json({ error: parsed.error.flatten() }, { status: 422 })
    }
    const appointment = await service.addPayment(
      tenant,
      appointmentId,
      parsed.data.amount,
    ...
    if (err instanceof NextResponse) return err
    console.error('[POST /api/haraka/appointments/[appointmentId]/payments]', err)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
