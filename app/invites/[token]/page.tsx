...
function AlertCircleSVG() {
  return (
    <svg width="22" height="22" viewBox="0 0 22 22" fill="none" aria-hidden>
      ...
      <path d="M11 7v4.5M11 13.5v.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
    </svg>
  );
}
function UserSVG() {
...
export default function AcceptInvitePage({ params }: { params: { token: string } }) {
  ...
  async function handleAccept(e: React.FormEvent) {
    ...
    try {
      ...
      // Force-refresh so custom claims (role, organizationId) set server-side are in the token
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
          ...
          <div className="space-y-2.5 text-sm mb-6 pb-5 border-b border-gray-100">
            ...
            <div className="flex items-center gap-2.5 text-gray-700">
              <span className="text-gray-400"><MailSVG /></span>
              <span>{info.email}</span>
            </div>
            <div className="flex items-center gap-2.5 text-gray-700">
              <span className="text-gray-400"><ShieldSVG /></span>
              <span className="capitalize">{info.role}</span>
              <span className="ml-auto text-xs font-medium px-2 py-0.5 rounded-full" style={{ background: info.role === 'admin' ? 'var(--primary-100)' : 'var(--gray-100)', color: info.role === 'admin' ? 'var(--primary-700)' : 'var(--gray-600)' }}>
                {info.role}
              </span>
            </div>
          </div>
          ...
          <form onSubmit={handleAccept} className="space-y-4">
            ...
            <Button type="submit" className="w-full h-11" disabled={submitting}>
              {submitting
                ? <span className="inline-flex items-center gap-2"><SpinnerSVG />Setting up your account…</span>
                : 'Accept invitation'}
            </Button>
          </form>
        </div>
