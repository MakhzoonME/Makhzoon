...
export function Cart() {
  ...
  if (lines.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-gray-500 text-sm gap-2">
        ...
        <div className="text-xs">Scan a barcode or pick a product on the left.</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      <div className="flex-1 overflow-y-auto -mr-1 pr-1 divide-y divide-border">
        {priced.lines.map((line) => (
          <div key={line.itemId} className="py-2 grid grid-cols-[1fr_auto_auto] gap-2 items-center">
            ...
            <div className="flex items-center gap-2">
              <div className="font-mono text-sm w-16 text-right">{line.lineTotal.toFixed(2)}</div>
              <Button
                size="sm"
                variant="ghost"
                className="h-7 w-7 p-0"
      ...
      <div className="border-t border-border pt-3 mt-3 space-y-1 text-sm font-mono">
        <Row label="Subtotal" value={priced.totals.subtotal} />
        <Row label="Discount" value={-priced.totals.discountTotal} dim={priced.totals.discountTotal === 0} />
        <Row label="Tax" value={priced.totals.taxTotal} dim={priced.totals.taxTotal === 0} />
