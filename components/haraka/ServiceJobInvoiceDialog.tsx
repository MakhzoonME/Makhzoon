  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh]">
        <DialogHeader>
          <DialogTitle>
            Service Invoice — {job.jobNumber}
            {job.invoiceNumber && <span className="text-gray-400 text-sm font-normal ml-2">({job.invoiceNumber})</span>}
          </DialogTitle>
        </DialogHeader>
        ...
        <DialogBody className="space-y-3">
          ...
          {!job.invoiceNumber && job.status !== 'done' && (
            <div className="rounded-lg border border-orange-200 bg-orange-50 px-4 py-3 text-xs text-orange-700">
              Invoice number is assigned once the job is marked <strong>Done</strong>.
            </div>
          )}
          {!job.invoiceNumber && job.status === 'done' && (
            <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-xs text-blue-700 flex items-center justify-between">
              <span>No invoice number yet. Generate one to include it on the document.</span>
              <Button size="sm" variant="outline" className="ml-3 h-7 text-xs" onClick={ensureInvoiceNumber} disabled={generateMut.isPending}>
                {generateMut.isPending ? <Loader2 size={12} className="animate-spin" /> : 'Generate'}
              </Button>
            </div>
          )}
