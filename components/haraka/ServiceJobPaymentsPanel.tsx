    ...
    <div className="rounded-xl border border-border bg-surface-page p-5 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold uppercase tracking-wider text-gray-400">{t('paymentPanel.sectionLabel')}</span>
        </div>
          ...
          {payments.map((p) => (
            <div key={p.id} className="flex items-center gap-2 text-sm">
              <div className="flex-1 flex items-center gap-2 text-gray-600">
                <span className="capitalize">{p.paymentMethod?.replace(/_/g, ' ') ?? t('paymentPanel.sectionLabel')}</span>
      ...
      <div className="rounded-lg bg-surface-card border border-border p-3 space-y-1.5 text-sm">
        <div className="flex justify-between text-gray-500">
          <span>{t('paymentPanel.jobTotal')}</span>
          ...
          {!showForm ? (
            ...
            <div className="space-y-3 pt-1">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-gray-600">{t('paymentPanel.amountLabel')} *</label>
                  <Input
