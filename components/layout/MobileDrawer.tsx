...
import { ORG_NAV_ENTRIES, NavGroupConfig } from '@/lib/nav';
...
export function MobileDrawer() {
  ...
  const visibleEntries = ORG_NAV_ENTRIES.filter((entry) => {
  ...
  function isGroupActive(group: NavGroupConfig) {
    return group.items.some((sub) => {
      const full = `/${orgSlug}${sub.href}`;
      ...
      {mobileMenuOpen && (
        <>
          ...
          <motion.aside
            ...
            className="fixed left-0 top-0 bottom-0 z-50 w-72 bg-white dark:bg-gray-900 flex flex-col shadow-xl md:hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-800">
              <div className="flex items-center gap-2">
                <MakhzoonMark size={24} />
                <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">Makhzoon</span>
              </div>
              <button
                type="button"
                onClick={() => setMobileMenuOpen(false)}
            ...
            {user && (
              <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-800">
                <div className="flex items-center gap-2">
                  <div className="h-8 w-8 rounded-full bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center text-sm font-semibold text-indigo-700 dark:text-indigo-300 flex-shrink-0">
                    {user.displayName?.[0]?.toUpperCase() ?? displayIdentity(user.email)?.[0]?.toUpperCase()}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{user.displayName || displayIdentity(user.email)}</p>
            ...
            <nav className="flex-1 overflow-y-auto p-2.5 space-y-0.5">
              {visibleEntries.map((entry) => {
                ...
                if ('type' in entry && entry.type === 'group') {
                  ...
                  const isOpen = openGroups[group.href] ?? active;
                  ...
                  return (
                    <div key={group.href}>
                      <button
                        type="button"
                        onClick={() => setOpenGroups((prev) => ({ ...prev, [group.href]: !prev[group.href] }))}
                        className={cn(
                      ...
                      >
                        ...
                        <span className={cn('transition-transform duration-200 opacity-60', isOpen && 'rotate-180')}>
                          <ChevronDownSVG />
                        </span>
                      </button>
                      ...
                      <AnimatePresence initial={false}>
                        {isOpen && (
                          ...
                          >
                            <div className="pl-10 space-y-0.5 pt-0.5">
                              {visibleSubItems.map((sub) => {
                                const fullHref  = `/${orgSlug}${sub.href}`;
                ...
                /* ── Regular item ───────────────────────────────── */
                const { href, label, featureKey, adminOnly } = entry as { href: string; label: string; featureKey?: string; adminOnly?: boolean };
                void featureKey; void adminOnly; // already filtered above
            ...
            <div className="p-3 border-t border-gray-100 dark:border-gray-800">
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/30 transition-colors duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <LogOutSVG />
