...
import {
  addAppointmentPaymentSchema,
  settleAppointmentPaymentSchema,
...
import { VERTICAL_KEYS, VERTICALS, VERTICAL_FEATURE_KEYS } from '@/lib/platform/verticals';
...
describe('visits repository/service shape', () => {
  // The repository/service modules are 'server-only' (they touch
  // supabaseAdmin), so this suite can't import the classes directly — that
  // would fail the same way importing them from a client component would.
  // Instead, assert the exact method surface each API route depends on
  // exists as SOURCE TEXT, catching an accidental rename/removal without
  // needing a live DB or a server-only-aware test environment.
  it('exposes the full visit CRUD + notes + attachments + follow-ups surface', () => {
    const repoSrc = readFileSync(
      join(process.cwd(), 'lib/modules/zeyara/visits/visits.repository.ts'),
      'utf-8',
    );
    const serviceSrc = readFileSync(
      join(process.cwd(), 'lib/modules/zeyara/visits/visits.service.ts'),
      'utf-8',
    );
    for (const method of [
      'list', 'getById', 'getByAppointmentId', 'create', 'update', 'delete',
      'listNotes', 'addNote',
      'listAttachments', 'addAttachment', 'getAttachment', 'deleteAttachment',
      'listFollowUps',
    ]) {
      expect(repoSrc, `VisitsRepository.${method} missing`).toMatch(new RegExp(`async ${method}\\(`));
    }
    for (const method of [
      'list', 'getById', 'getByAppointmentId', 'create', 'update', 'delete',
      'listNotes', 'addNote',
      ...
      expect(serviceSrc, `VisitsService.${method} missing`).toMatch(new RegExp(`async ${method}\\(`));
    }
  });
});

describe('visit RLS matches haraka_appointments\' shape', () => {
  // Design requires platform-admin all / org-manager all / org-member select
  // on every clinical table — the same shape as haraka_appointments, not a
  // broader one (per-provider record isolation is explicitly deferred).
  // Read the migration SQL directly since there is no live DB in this suite.
  const sql = readFileSync(
    join(process.cwd(), 'supabase/migrations/0082_zeyara_visits.sql'),
    'utf-8',
  );

  it.each(['zeyara_visits', 'zeyara_visit_notes', 'zeyara_visit_attachments'])(
    '%s has platform-admin ALL, org-manager ALL, and org-member SELECT policies',
    (table) => {
      expect(sql, `${table} RLS not enabled`).toMatch(new RegExp(`ALTER TABLE ${table} ENABLE ROW LEVEL SECURITY`));
      expect(sql, `${table} missing platform-admin policy`).toMatch(
        new RegExp(`CREATE POLICY \\w+ ON ${table}\\s+FOR ALL USING \\(public\\.is_platform_admin\\(\\)\\)`),
      );
      expect(sql, `${table} missing org-manager policy`).toMatch(
        new RegExp(`CREATE POLICY \\w+ ON ${table}\\s+FOR ALL USING \\(public\\.is_org_manager\\(organization_id\\)\\)`),
      );
      expect(sql, `${table} missing org-member SELECT policy`).toMatch(
        new RegExp(`CREATE POLICY \\w+ ON ${table}\\s+FOR SELECT USING \\(public\\.belongs_to_org\\(organization_id\\)\\)`),
      );
    },
  );

  it('an appointment carrying a clinical record cannot be deleted (ON DELETE RESTRICT)', () => {
    expect(sql).toMatch(/appointment_id\s+uuid NOT NULL REFERENCES haraka_appointments\(id\) ON DELETE RESTRICT/);
  });
});

describe('custom fields reach bookings and clinical records', () => {
  // The Zeyara flag is worthless if the field-definition validator still
  // rejects the new scopes — the DB constraint alone would let a row in that
...
describe('the vertical-OR gate is additive, not a replacement', () => {
  it('registers zeyara alongside haraka, not instead of it', () => {
    expect(VERTICAL_KEYS).toContain('haraka');
    expect(VERTICAL_KEYS).toContain('zeyara');
    expect(VERTICALS.zeyara.featureKey).toBe('zeyara');
    expect(VERTICALS.zeyara.permModule).toBe('zeyara');
    expect(VERTICAL_FEATURE_KEYS).toEqual(expect.arrayContaining(['pos', 'zeyara']));
  });

  it('zeyara defaults OFF for an org that never configured it — existing orgs are unaffected', () => {
    // The whole "ship dark, flip per-subscription" plan depends on this: an
    // org's stored feature map predates 'zeyara' entirely, and an absent key
    // must resolve to false, not true, for a newly-added VERTICAL.
    expect(OPT_IN_FEATURE_KEYS).toContain('zeyara');
    expect(FEATURE_KEYS).toContain('zeyara');
  });

  it('a shared-engine grant on EITHER namespace alone satisfies the OR gate', () => {
    const zeyaraOnly = staffWith({ zeyara: { appointmentsCreate: true } });
    const harakaOnly = staffWith({ haraka: { appointmentsCreate: true } });
    expect(hasAnyVerticalPermission(zeyaraOnly, 'appointmentsCreate')).toBe(true);
    expect(hasAnyVerticalPermission(harakaOnly, 'appointmentsCreate')).toBe(true);
  });

  it('grants nothing when neither namespace holds the op', () => {
    const neither = staffWith({ zeyara: { appointmentsView: true } });
    expect(hasAnyVerticalPermission(neither, 'appointmentsCreate')).toBe(false);
  });
});

describe('split-payment status transitions (appointments, shared with Haraka)', () => {
  it('addPayment defaults to no status field — the API applies "paid" server-side for back-compat', () => {
    const parsed = addAppointmentPaymentSchema.safeParse({ amount: 50 });
    expect(parsed.success).toBe(true);
    if (!parsed.success) return;
    expect(parsed.data.status).toBeUndefined();
  });

  it('accepts an explicit "unpaid" line — e.g. an insurer share not yet collected', () => {
    const parsed = addAppointmentPaymentSchema.safeParse({ amount: 50, status: 'unpaid' });
    expect(parsed.success).toBe(true);
    if (!parsed.success) return;
    expect(parsed.data.status).toBe('unpaid');
  });

  it('rejects "written_off" as an initial payment status — a line is only written off by settling it', () => {
    const parsed = addAppointmentPaymentSchema.safeParse({ amount: 50, status: 'written_off' });
    expect(parsed.success).toBe(false);
  });

  it('settle accepts only the two terminal outcomes for an unpaid line', () => {
    expect(settleAppointmentPaymentSchema.safeParse({ status: 'paid' }).success).toBe(true);
    expect(settleAppointmentPaymentSchema.safeParse({ status: 'written_off' }).success).toBe(true);
    expect(settleAppointmentPaymentSchema.safeParse({ status: 'unpaid' }).success).toBe(false);
    expect(settleAppointmentPaymentSchema.safeParse({}).success).toBe(false);
  });
});
